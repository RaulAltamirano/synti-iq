...
import {
  TWO_FA_SETUP_REDIS_TTL_SECONDS,
  twoFaSetupRedisKey,
} from 'src/auth/constants/auth-cache.constants';
...
export class TwoFactorService {
  ...
  async generateSecretForUserId(userId: string): Promise<{ secret: string; qrCode: string }> {
  ...
  async verifyAndActivateForUserId(
    userId: string,
    token: string,
    ...
    const result = await this.verifyAndActivate(user, token);
    return { backupCodes: result.backupCodes };
  }

  async getStatusForUserId(userId: string): Promise<TwoFactorStatus> {
  ...
  async regenerateBackupCodesIfEnabled(userId: string): Promise<string[]> {
    const status = await this.getStatusForUserId(userId);
    if (!status.enabled) {
  ...
  async verifyAndActivate(user: User, token: string): Promise<VerifyAndActivateResult> {
    return this.observabilityService.withSpan(AUTH_2FA_SPAN_NAMES.VERIFY_ACTIVATE, async span => {
      span.setAttribute(AUTH_2FA_SPAN_ATTRIBUTES.USER_ID, user.id);
      const tempSecret = await this.redisService.get(twoFaSetupRedisKey(user.id));
      if (!tempSecret) {
      ...
      await this.redisService.del(twoFaSetupRedisKey(user.id));
  ...
  async getStatus(user: Pick<User, 'id'>): Promise<TwoFactorStatus> {
