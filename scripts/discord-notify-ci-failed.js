#!/usr/bin/env node
/**
 * Notifies Discord when CI quality gate fails.
 * Env: DISCORD_WEBHOOK
...
async function main() {
  ...
  if (!webhook) {
    console.log('DISCORD_WEBHOOK not set — skipping CI failed notification');
    return;
  }
  validateWebhook(
    webhook,
    'Invalid DISCORD_WEBHOOK. CI failed notifications require the secret in Settings > Secrets.',
  );
