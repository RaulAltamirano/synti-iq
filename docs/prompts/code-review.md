...
**Placeholder:** Replace "the following file(s) or module" with the actual path (e.g. `src/store/store.service.ts` or `src/store/`).
...
Verify the review output uses the specified format (PASS/FAIL/N/A, Location, Detail, Reference) and includes the summary count.
